        <div class="region region-footer row">
            <section class="block col-sm-3">
                <h2 class="block-title">ИНФОРМАЦИЯ</h2>
                <p>
                    <a href="mailto:grandmoto@ukr.net">grandmoto@ukr.net</a><br>
                    Тел:(093) 359-44-14 Станислав.<br>
                    11:00-24:00 пн-пт<br>
                    Отправка заказов в пн, ср, пт в 17:00
                </p>
            </section>
            <section class="block col-sm-3">
                <h2 class="block-title">ЦЕНТР ПОДДЕРЖКИ</h2>
                <ul class="menu nav">
                    <li class="first leaf"><a title="" href="/news">Новости</a></li>
                    <li class="leaf"><a title="" href="/articles">Полезные статьи</a></li>
                    <li class="leaf"><a title="" href="/podarochnye-sertifikaty">Подарочные сертификаты</a></li>
                    <li class="last leaf"><a title="" href="/kredit-i-rassrochka">Кредит и рассрочка</a></li>
                </ul>
            </section>
            <section class="block col-sm-3">
                <h2 class="block-title">АКЦИИ &amp; НОВОСТИ</h2>
                <div class="menu-block-wrapper menu-block-1 menu-name-menu-secondmenu parent-mlid-0 menu-level-1">
                    <ul class="menu nav">
                        <li class="first leaf menu-mlid-1109"><a title="" href="/actions">Акции</a></li>
                        <li class="leaf menu-mlid-1110"><a title="" href="/dostavka-i-oplata">Как купить</a></li>
                        <li class="leaf menu-mlid-1111"><a title="" href="/o-nas">О нас</a></li>
                        <li class="last leaf menu-mlid-1112"><a title="" href="/kak-dobratsya">Как добраться?</a></li>
                    </ul>
                </div>
            </section>
            <section class="block col-sm-3">
                <h2 class="block-title">СОЦИАЛЬНЫЕ СЕТИ</h2>
                <div class="images">
                    <a href="">vk</a>
                </div>
            </section>
        </div>